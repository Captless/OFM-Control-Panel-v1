...
function _setSettingsStatus(msg, type) {
  ...
  if (!el) return;
  if (!msg) { el.textContent = ''; el.className = 'field-status'; return; }
  el.textContent = msg;
...
async function loadSettings() {
  ...
  if (urlInput) urlInput.value = identity.avatar_url || '';
}
...
async function handleAvatarFile(file) {
  ...
  try {
    ...
    if (!data.ok) { _setSettingsStatus(data.error || 'Upload failed', 'error'); return; }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var zone = document.getElementById('settings-upload-zone');
  if (fileInput) {
    fileInput.addEventListener('change', function() { handleAvatarFile(fileInput.files[0]); });
  }
  if (zone) {
