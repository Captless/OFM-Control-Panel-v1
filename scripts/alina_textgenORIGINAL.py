...
Produces on-screen + post captions grounded in Alina's alt-girl identity:
raw amateur iPhone aesthetic, damp/wet hair, black band tees, 3am texts,
mirror checks, fit checks, night drives, dark rooms, moody candid energy.

Structure: OPENERS (5 hooks x 20) -> MIDDLES (5 hooks x 12) -> CLOSERS (5 hooks x 12).
Each caption has an on-screen variant (opener only, short) and post variant (full).

Usage:
  python scripts/alina_textgen.py 10
  python scripts/alina_textgen.py 5 --hook playful --seed 42
"""
...
# ---------------------------------------------------------------------------
# Unified CTA + hashtag pools (no platform distinction).
# ---------------------------------------------------------------------------
...
HASHTAG_POOL = [
    ...
    "#aesthetic", "#reels", "#explore", "#foryou",
    "#shorts", "#youtubeshorts", "#alt",
]
...
def generate_caption(hook_type=None, seed=None, _rng=None):
    """Generate one caption dict.

    hook_type: one of the 5 keys; None or unknown -> random hook.
    seed: private Random(seed) for thread-safe reproducibility.
    _rng: internal — callers with seed should pass explicit Random instance.
    Returns: {"on_screen", "post", "hook_type", "cta", "hashtags"}
    """
    rng = _rng or random
    ...
    # On-screen: opener only (short, punchy)
    on_screen = opener.strip().rstrip(".") + "."

    # Post: full conversational (opener + middles + closer)
    parts = [opener] + middles + [closer]
    post = ". ".join(p.rstrip(".") for p in parts)
    if not post.endswith((".", "!", "?")):
        post += "."

    num_hashtags = rng.randint(3, 6)
...
def batch_generate(count, hook_types=None, seed=None):
    """Generate `count` unique captions.

    hook_types: list of hook keys to pick from; None -> all 5.
    seed: private Random(seed) for thread-safe reproducibility.
    Dedups by on_screen text — collisions regenerate, guarded by cap.
...
def main():
    ...
    parser.add_argument("--hook", choices=_HOOK_KEYS, default=None,
    ...
    for i, cap in enumerate(
        batch_generate(args.n, hook_types=hook_types, seed=args.seed), 1
    ):
